...
def _print(args, raw=False):
    ...
    for _dict in args:
        if Keys.text in _dict:
            if Keys.alert in _dict and _dict[Keys.alert] == True:
                final_string += '\n'
            ...
            if Keys.bold in _dict and _dict[Keys.bold] == True:
                final_string += Color.BOLD
            if Keys.underline in _dict and _dict[Keys.underline] == True:
                final_string += Color.UNDERLINE
            if Keys.alert in _dict and _dict[Keys.alert] == True:
                final_string += line + (_dict[Keys.text]).center(int(columns), ' ') + line
            else:
...
def colorize(text, color, bold=False, underline=False, background=None, alert=False, end='\n', raw=False):
    _string = _print([{
        Keys.text : text,
        Keys.color : color,
